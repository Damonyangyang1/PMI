	...
	while(*s) 
	{
		if( *s < 0x80 ) 
		{
			...
			for(i=0;i<24;i++)
			{
				...
				for(j=0;j<4;j++) 
				{
					if(asc24[k].Msk[i*2+1]&(0x80>>j))	Gui_DrawPoint(x+j+8,y+i,fc);
					else {
						if (fc!=bc) Gui_DrawPoint(x+j+8,y+i,bc);
					}
				}
...
void Gui_DrawFont_GBK32(uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, uint8_t *s)
{
	unsigned char i,j;
	unsigned short k;
		...
		for (k=0;k<hz40_num;k++) 
		{
		  ...
		  { 
				for(i=0;i<32;i++)
				{
					...
					for(j=0;j<8;j++) 
						{
							if(hz40[k].Msk[i*2+1]&(0x80>>j))	Gui_DrawPoint(x+j+8,y+i,fc);
							else {
								if (fc!=bc) Gui_DrawPoint(x+j+8,y+i,bc);
							}
						}				
...
void Gui_DrawFont_GBK36(uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, uint8_t *s)
{
	unsigned char i,j;
	unsigned short k;
		...
		for (k=0;k<hz32_num;k++) 
		{
		  ...
		  { 
				for(i=0;i<32;i++)
				{
					...
					for(j=0;j<8;j++) 
						{
							if(hz32[k].Msk[i*4+2]&(0x80>>j))	Gui_DrawPoint(x+j+16,y+i,fc);
							else {
								if (fc!=bc) Gui_DrawPoint(x+j+16,y+i,bc);
							}
						}
					for(j=0;j<8;j++) 
						{
							if(hz32[k].Msk[i*4+3]&(0x80>>j))	Gui_DrawPoint(x+j+24,y+i,fc);
							else {
